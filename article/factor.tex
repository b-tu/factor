...
One of the main problems in active portfolio management is concerned with the modelling of excess returns \eqref{eqn:relative_return}. In particular, one often tries to model the excess returns of the $N$ assets, at time $t > 0$, as a function of the information set:
\begin{equation}
...
% \begin{remark}
% 	[Excess asset returns] The unknown parameters of the multi-factor model \eqref{eqn:multi_factor_model} are usually estimated based on the individually observed excess asset returns $\mathcal{A}_n \in \mathbb{A}$ and not the returns of the different portfolios $\mathcal{I} \in \mathbb{I}$. Intuitively, this approach makes sense when the asset excess returns are observables as this reduces the number of parameters that we should estimate per asset. \textcolor{magenta}{Question to self: Are there occasions when using the portfolio excess returns makes more sense?}
% \end{remark}
